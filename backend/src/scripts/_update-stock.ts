 ...
 * _update-stock.ts
 *
 * Reads "new-stock.xlsx" and UPDATES product stock in the CRM database.
 * Uses the SAME matching logic as _check-stock.ts (SKU bridge, 4-pass matching,
 * deduplication of Excel rows).
 *
 * For each matched product where stock differs:
 *   - Updates products.stock = newStock
 *   - Creates an inventory_movements record with type='CORRECTION'
 *
 * Usage:
 *   npx tsx src/scripts/_update-stock.ts
 */
...
function readExcelFile(filePath: string): ExcelProduct[] {
  ...
  for (const sheetName of wb.SheetNames) {
    ...
    console.log(`    Detected: headerEndRow=${headerEndRow}, nameCol=${nameCol}, formatCol=${formatCol}, unitCol=${unitCol}, stockCol=${stockCol}`);
...
async function main() {
  const excelPath = path.resolve(__dirname, '../../../new_stock_for-14.03.2026.xlsx');
  console.log(`\n  STOCK UPDATE SCRIPT`);
  ...
  // Find an admin user for created_by on inventory_movements
  const adminUser = await prisma.user.findFirst({
    where: { role: { in: ['SUPER_ADMIN', 'ADMIN'] }, isActive: true },
    select: { id: true, fullName: true },
  });
  if (!adminUser) {
    console.error('\n  ERROR: No active SUPER_ADMIN or ADMIN user found. Cannot create inventory movements.');
    process.exit(1);
  }
  console.log(`  Using admin user for movements: ${adminUser.fullName} (${adminUser.id})`);
  ...
  // Determine which matched products need updating (stock differs)
  const toUpdate = matched.filter(r => {
    const diff = r.excelProduct.stock - r.crmProduct!.stock;
    return Math.abs(diff) >= 0.001; // tolerance for floating-point
  });
  ...
  console.log(`  Need stock update:  ${toUpdate.length}`);
  console.log(`  Already correct:    ${matched.length - toUpdate.length}`);

  if (toUpdate.length === 0) {
    console.log(`\n  Nothing to update. All stock values match.\n`);
    return;
  }

  // Sort by CRM name for display
  toUpdate.sort((a, b) => (a.crmProduct?.name ?? '').localeCompare(b.crmProduct?.name ?? '', 'ru'));

  // Execute updates inside a Prisma transaction
  console.log(`\n${'='.repeat(140)}`);
  console.log(`  APPLYING STOCK UPDATES (${toUpdate.length} products)`);
  console.log(`${'='.repeat(140)}`);
  console.log(
    `  ${pad('#', 4)} ${pad('Product Name', 50)} ${pad('SKU', 28)} ${rpad('Old', 10)} ${rpad('New', 10)} ${rpad('Diff', 10)} ${pad('Match', 12)}`
  );
  console.log(
    `  ${'-'.repeat(4)} ${'-'.repeat(50)} ${'-'.repeat(28)} ${'-'.repeat(10)} ${'-'.repeat(10)} ${'-'.repeat(10)} ${'-'.repeat(12)}`
  );
  ...
  const NOTE = 'Stock update from new_stock_for-14.03.2026.xlsx (March 2026)';

  // Pre-compute diffs and log BEFORE the transaction to avoid timeout
  const updateOps: { productId: string; newStock: number; absDiff: number; }[] = [];
  for (let i = 0; i < toUpdate.length; i++) {
    const r = toUpdate[i];
    const cp = r.crmProduct!;
    const ep = r.excelProduct;
    const oldStock = cp.stock;
    const newStock = ep.stock;
    const diff = newStock - oldStock;
    const diffStr = diff > 0 ? `+${diff}` : String(diff);

    if (diff > 0) { totalIncrease += diff; countIncrease++; }
    else { totalDecrease += Math.abs(diff); countDecrease++; }

    updateOps.push({ productId: cp.id, newStock, absDiff: Math.abs(diff) });

    const qualityStr = r.quality + (r.detail ? ` ${r.detail}` : '');
    console.log(
      `  ${pad(String(i + 1), 4)} ${pad(cp.name, 50)} ${pad(cp.sku, 28)} ${rpad(String(oldStock), 10)} ${rpad(String(newStock), 10)} ${rpad(diffStr, 10)} ${pad(qualityStr, 12)}`
    );
  }

  // Execute all updates in a single transaction with extended timeout
  console.log(`\n  Applying ${updateOps.length} updates in transaction...`);
  await prisma.$transaction(async (tx) => {
    for (const op of updateOps) {
      await tx.product.update({
        where: { id: op.productId },
        data: { stock: op.newStock },
      });
  ...
  console.log(`${'='.repeat(70)}`);
  console.log(`  Total products updated:  ${toUpdate.length}`);
  console.log(`  Stock increases:         ${countIncrease} (total +${Math.round(totalIncrease * 100) / 100})`);
  console.log(`  Stock decreases:         ${countDecrease} (total -${Math.round(totalDecrease * 100) / 100})`);
  console.log(`  Net stock change:        ${totalIncrease - totalDecrease > 0 ? '+' : ''}${Math.round((totalIncrease - totalDecrease) * 100) / 100}`);
  console.log(`  Inventory movements:     ${toUpdate.length} CORRECTION records created`);
  console.log(`  Note on movements:       "${NOTE}"`);
  console.log(`\n  *** LIVE UPDATE APPLIED SUCCESSFULLY ***\n`);
}
