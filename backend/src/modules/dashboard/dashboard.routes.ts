...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const [
      ...
      // 5. Total debt (same as /finance/debts totals: Общий долг = sum of all clients with positive debt)
      dealScope.managerId
        ? prisma.$queryRaw<{ total_gross_debt: string }[]>(
            Prisma.sql`SELECT COALESCE(SUM(client_debt), 0)::text as total_gross_debt
            FROM (
              SELECT SUM(amount - paid_amount) as client_debt
              FROM deals
    ...
    const grossDebt = Number(debtSplit?.total_gross_debt ?? 0);
