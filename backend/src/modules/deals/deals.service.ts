...
function assertPaymentDeletable(
  ...
  if (isClientCreditTransfer(payment)) {
    throw new AppError(
      ...
      + 'сумму сделкам-источникам. Нужно сторнирование — SUPER_ADMIN может сделать это '
      + 'кнопкой «Сторнировать» на карточке сделки.',
    );
  }
...
export class DealsService {
  ...
  async findForStockConfirmation(_user: AuthUser) {
    const deals = await prisma.deal.findMany({
      ...
      include: {
        ...
        items: {
          include: {
            product: { select: { id: true, name: true, sku: true, unit: true, category: true, stock: true, rollStock: true, salePrice: true } },
          },
        },
      },
      orderBy: { createdAt: 'desc' },
  ...
  async submitShipment(dealId: string, dto: ShipmentDto, user: AuthUser) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const title = retitleOnCloseIfDefault(deal, closedAt);
      await tx.deal.update({
        where: { id: dealId },
        data: { status: targetStatus, closedAt, title },
      });
    });
  ...
  async warehouseManagerConfirm(dealId: string, user: AuthUser) {
    ...
    const title = retitleOnCloseIfDefault(deal, closedAt);
    await prisma.$transaction(async (tx) => {
      await this.deductInventoryForDealInTx(tx, dealId, user.userId, 'Автосписание при закрытии зав. складом');
      await tx.deal.update({ where: { id: dealId }, data: { status: 'CLOSED', closedAt, title } });
    });
  ...
  async approveByAdmin(dealId: string, user: AuthUser) {
    ...
    const title = retitleOnCloseIfDefault(deal, closedAt);
    await prisma.$transaction(async (tx) => {
      await this.deductInventoryForDealInTx(tx, dealId, user.userId, 'Автосписание при одобрении админом');
      await tx.deal.update({ where: { id: dealId }, data: { status: 'CLOSED', closedAt, title } });
    });
  ...
  async markLoaded(dealId: string, user: AuthUser) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (targetStatus === 'CLOSED') {
        ...
        await tx.deal.update({
          where: { id: dealId },
          data: { status: 'CLOSED', closedAt, title },
        });
      } else {
