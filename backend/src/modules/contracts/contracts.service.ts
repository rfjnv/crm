...
export class ContractsService {
  ...
  async findById(id: string) {
    const contract = await prisma.contract.findFirst({
      ...
      include: {
        client: { select: { id: true, companyName: true, contactName: true, phone: true, address: true, isSvip: true, creditStatus: true } },
        deals: {
          select: {
  ...
  async generatePdf(id: string, docType: DocType = 'CONTRACT', poaId?: string): Promise<Buffer> {
    const contract = await prisma.contract.findFirst({
      ...
      include: {
        client: {
          select: {
            id: true, companyName: true, contactName: true, phone: true, address: true, isSvip: true, creditStatus: true,
